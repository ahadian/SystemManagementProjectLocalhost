		...
		if(empty($error_mg)){		
			if(EI_MODE == 'demo'){
				...
				if(EI_DATABASE_CREATE && ($install_type == 'create') && !$db->Create()){
					...
					if(rename("../../index.php","../../index_backup.php")){
						$location = str_replace("install/complete_installation.php","",$_SERVER['SCRIPT_NAME']);
						$myfile2 = fopen("../../index.php", "w") or die("Unable to open / create index.php xampp!");
						$txt2 = "<?php header('location:".$location."');?>";
						fwrite($myfile2, $txt2);
						fclose($myfile2);
					}
				}else{
